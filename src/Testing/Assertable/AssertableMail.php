...
class AssertableMail
{
    ...
    public function __construct(PendingMail $mail_event, ViewFactoryInterface $view_factory)
    {
    ...
    public function assertTo($user_email, bool $ignore_count = false) :AssertableMail
    {
        ...
        $expected_recipients->each(function (string $email) use ($actual_recipients) {
            PHPUnit::assertTrue(
                $actual_recipients->containsStrict($email),
                "The email was not sent to [$email]."
            );
        });
    ...
    public function assertViewHas(array $data) :AssertableMail
    {
        foreach ($data as $key => $value) {
            if (is_int($key)) {
                PHPUnit::assertTrue(
                    Arr::has($this->mail->buildViewData(), $value),
                    "The mail view does not have any context for [$value]."
                );
            }
            else {
                PHPUnit::assertTrue(
                    Arr::has($this->mail->buildViewData(), $key),
                    "The mail view does not have any context for [$key]."
                );
