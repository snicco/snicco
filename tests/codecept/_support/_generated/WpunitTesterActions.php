...
declare(strict_types=1);  //[STAMP] 035d08ce82be7813ea899c5dfb2528a7
...
trait WpunitTesterActions
{
     ...
     * The value will first look at the `WP_PLUGIN_DIR` constant, then the `pluginsFolder`
     * configuration parameter and will, finally, look in the default path from the WordPress root
     * directory.
     *
     * @param  string  $path  A relative path to append to te plugins directory absolute path.
     *
     * @return string The absolute path to the `pluginsFolder` path or the same with a relative
     *     path appended if `$path` is provided.
    ...
    public function getPluginsFolder($path = "")
    {
        return $this->getScenario()->runStep(
            new Action('getPluginsFolder', func_get_args())
        );
    }
     ...
     * [Core test
     * suite](https://make.wordpress.org/core/handbook/testing/automated-testing/writing-phpunit-tests/).
     *
    ...
    {
        return $this->getScenario()->runStep(
            new Action('factory', func_get_args())
        );
    }
    ...
    {
        return $this->getScenario()->runStep(
            new Action('getContentFolder', func_get_args())
        );
    }
     ...
     *
     * @param  callable|null  $format  A callback function to format the arguments debug output;
     *     the callback will receive the array of arguments as input.
     *
     * @return void
     * @example
     * ```php
    ...
    {
        return $this->getScenario()->runStep(
            new Action('startWpFiltersDebug', func_get_args())
        );
    }
    ...
    {
        return $this->getScenario()->runStep(
            new Action('stopWpFiltersDebug', func_get_args())
        );
    }
     ...
     * The output will show following the selected output verbosity (`--debug` and `-vvv` CLI
     * options).
     *
    ...
    {
        return $this->getScenario()->runStep(
            new Action('debugWpFilterInitial', func_get_args())
        );
    }
    ...
    {
        return $this->getScenario()->runStep(
            new Action('debugWpFilterFinal', func_get_args())
        );
    }
    ...
    {
        return $this->getScenario()->runStep(
            new Action('debugWpActionInitial', func_get_args())
        );
    }
