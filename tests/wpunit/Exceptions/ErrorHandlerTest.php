<?php
	...
	class ErrorHandlerTest extends WPTestCase {
		...
		public function testGetResponse_NotFoundException_404Response() {
			...
			$this->assertEquals( $expected, $this->subject->getResponse( $request, $exception )
			                                              ->getStatusCode() );
		}
		...
		public function testGetResponse_DebugException_PrettyErrorResponse() {
			...
			$this->assertEquals( $expected, $subject->getResponse( $request, $exception )->getBody()
			                                        ->read( strlen( $expected ) ) );
		}
