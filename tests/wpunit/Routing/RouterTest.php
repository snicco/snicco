	...
	class RouterTest extends WPTestCase {
		...
		public function testMergeMethodsAttribute() {
			...
			$this->assertEquals( [
				'foo',
				'bar',
			], $this->subject->mergeMethodsAttribute( [ 'foo' ], [ 'bar' ] ) );
		}
		...
		public function testMergeMiddlewareAttribute() {
			...
			$this->assertEquals( [
				'foo',
				'bar',
			], $this->subject->mergeMiddlewareAttribute( [ 'foo' ], [ 'bar' ] ) );
		}
		...
		public function testRoute_Group_MergedAttributes() {
			...
			$this->assertEquals( '/foo/{foo}/bar/{bar}/', $route->getAttribute( 'condition' )
			                                                    ->getUrl() );
			$this->assertEquals( [
				'foo' => '/^foo$/',
				'bar' => '/^bar$/',
			], $route->getAttribute( 'condition' )->getUrlWhere() );
			$this->assertEquals( [ 'foo', 'bar' ], $route->getAttribute( 'middleware' ) );
			$this->assertEquals( 'foo.bar', $route->getAttribute( 'name' ) );
			$this->assertSame( $route_attributes['handler'], $route->getAttribute( 'handler' )
			                                                       ->get() );
