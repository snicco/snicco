<?php
    ...
    class DatabaseMagicLinkTest extends IntegrationTest
    {
        ...
        private function createTables()
        {
            ...
            $this->db->query("CREATE TABLE `wp_magic_links` (
  ...
  KEY `sessions_last_activity_index` (`expires`)
        ...
        public function magic_link_usage_can_be_checked () {
            ...
            $request = TestRequest::fromFullUrl("GET", "https://foo.com/foo?expires=$expires&signature=$signature");
        ...
        public function garbage_collection_works () {
            ...
            $signature2 = $this->magic_link->create('/bar', $this->expires->addSeconds(100)->getTimestamp(), $this->request);
        ...
        public function a_magic_link_with_the_same_signature_is_not_stored_twice () {
            ...
            $query = $this->db->prepare("SELECT COUNT(*) FROM $this->table WHERE signature = %s", md5($signature));
        ...
        private function assertSeeLink(string $signature)
        {
            ...
            $query = $this->db->prepare("SELECT EXISTS(SELECT 1 FROM $this->table WHERE signature = %s LIMIT 1)", $signature);
